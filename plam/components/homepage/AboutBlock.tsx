...
function AboutBlock({ data }: { data: Data }) {
  const imgUrl = `http://localhost:1337${data.data.attributes.img.data.attributes.url}`;

  const imgLoader = () => imgUrl;
        ...
        <div className={styles.about__content}>
          ...
          <div className={styles.about__info} data-aos="fade-up">
            <div className={styles.about__text_block}>
              <ReactMarkdown className={[styles.text_bold, styles.text].join(' ')}>{data ? data.data.attributes.heading : ''}</ReactMarkdown>
